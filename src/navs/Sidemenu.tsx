...
const Sidemenu = () => {
 ...
 const getCreatorButton = () => {
  ...
  return (
    <>
      <style>{`
        .sb-overlay{position:fixed;inset:0;z-index:199;background:transparent;pointer-events:none;transition:background .3s;}
        .sb-overlay.open{background:rgba(0,0,0,.65);pointer-events:all;backdrop-filter:blur(2px);}
        .new-sidebar{position:fixed;top:0;right:0;width:82%;max-width:300px;height:100%;z-index:200;background:#0d1120;border-left:1px solid rgba(255,255,255,0.08);transform:translateX(100%);transition:transform .35s cubic-bezier(.4,0,.2,1);display:flex;flex-direction:column;overflow:hidden;}
