...
export default function CreatorPage() {
  ...
  // Step 1: Map raw data to card format
  const list: CreatorCardProps[] = useMemo(() => {
    return myCreators.map((m) => {
      const card = mapToCard(m);
      return {
  ...
  const sortedList = useMemo(() => {
    // If no sort option is selected, return the default sorted list (preserves backend order)
    ...
    return sorted.sort((a, b) => {
      const sortByOption = filterState.sortBy || "";
      ...
      switch (sortByOption) {
        ...
        default:
          // Default: Return as-is (preserves backend order)
          return 0;
      }
    });
  }, [filteredList, filterState.sortBy]);
