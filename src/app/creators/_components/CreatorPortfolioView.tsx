...
import { CreatorReview } from "./Creator_review";
...
import CreatorByIdNav from "./CreatorByIdNav";
import FollowStrip from "./FollowStrip";
import { formatCreatorPrices } from "./formatCreatorPrices";
...
export default function CreatorPortfolioView({ creatorPortfolioId }: { creatorPortfolioId: string }) {
  const Creator = [creatorPortfolioId];
...
@media(max-width:768px){.mcp-nav-actions{transform:translateX(20px);}}
  ...
  >
    {dcb ? "⏳ Processing..." : removeCrush ? "💖 Crushing" : `🎯 ${crush_text}`}
        ...
        <div className="mcp-price-card">
          ...
          <span>{`${creatorPriceValue} Gold`}</span>
