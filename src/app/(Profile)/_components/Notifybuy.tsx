...
export default function Notifybuy({
  ...
  buy: () => void;
  cancel: () => void;
  /** Optional: control the loading state externally instead of internally. */
  isProcessing?: boolean;
  /** Optional: shows a "not enough Gold" state with a Buy Gold CTA instead of Confirm. */
  insufficientBalance?: boolean;
  currentBalance?: number;
  onBuyGold?: () => void;
}) {
  const [internalProcessing, setInternalProcessing] = useState(false);
  const isPurchasing = isProcessingProp ?? internalProcessing;

  const handleConfirm = () => {
    if (isProcessingProp === undefined) setInternalProcessing(true);
    buy();
  };
    ...
    <div className="relative w-full max-w-[320px] bg-[#111624] border border-white/10 rounded-2xl overflow-hidden shadow-2xl">
      <div
        className={`absolute -top-16 left-1/2 -translate-x-1/2 w-40 h-40 rounded-full blur-3xl pointer-events-none ${
          insufficientBalance ? "bg-red-500/15" : "bg-[#f5c451]/20"
        }`}
      />
      ...
      <div className="relative px-6 pt-7 pb-6 flex flex-col items-center text-center">
        <div
          className={`w-14 h-14 rounded-full flex items-center justify-center mb-4 ${
            insufficientBalance
              ? "bg-gradient-to-br from-red-500 to-red-600 shadow-[0_8px_24px_-6px_rgba(239,68,68,0.5)]"
              : "bg-gradient-to-br from-[#f5c451] to-[#e8a93a] shadow-[0_8px_24px_-6px_rgba(245,196,81,0.55)]"
          }`}
        >
          {insufficientBalance ? (
        ...
        <h3 className="text-white text-[16px] font-bold mb-1.5">
          {insufficientBalance ? "Not enough Gold" : "Unlock this content?"}
        </h3>
        <p className="text-gray-400 text-[13px] leading-relaxed mb-5">
          {insufficientBalance
            ? "Top up your Gold balance to unlock this content."
            : "This amount will be deducted from your Gold balance."}
        </p>

        <div className="w-full flex items-center justify-center gap-2 bg-white/[0.04] border border-white/10 rounded-xl py-3 mb-3">
        ...
        {insufficientBalance ? (
          <div className="w-full bg-red-500/10 border border-red-500/25 rounded-xl px-3.5 py-2.5 mb-6 text-left">
            <p className="text-red-300 text-[12px]">
              You have <span className="font-semibold text-red-200">🪙 {(currentBalance ?? 0).toFixed(2)}</span> — you need{" "}
              <span className="font-semibold text-red-200">
                🪙 {(parseFloat(String(price)) - (currentBalance ?? 0)).toFixed(2)}
              </span>{" "}
              more.
            </p>
          </div>
        ...
        <div className="w-full flex gap-3">
          ...
          {insufficientBalance ? (
            <button
              type="button"
              onClick={onBuyGold}
              className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-[#f5c451] to-[#e8a93a] text-black text-[13.5px] font-bold hover:brightness-105 transition-all"
            >
              Buy Gold
            </button>
          ) : (
