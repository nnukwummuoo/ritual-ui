...
export const ListofModels: React.FC<ListofModelsProps> = ({
  photolink,
  hosttype,
  online,
  name,
  ...
  const [modelID] = useState([modelid, userid]);
  ...
  return (
    <li
      ...
      onClick={() => {
        router.push(`/modelbyid/${modelID.toString()}`);
      }}
    >
      <div className="relative">
        <div>
          <Image
            alt="verified"
            src={hostimg || DummyModelImage}
            width={400}
            height={320}
            className="object-cover w-full rounded h-80"
            onError={() => sethostimg(DummyModelImage.src)}
          />
        </div>
