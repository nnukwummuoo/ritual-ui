...
import { unverifiedHost, changemodelstatus } from '@/store/modelSlice';
// import { Hostlist } from '../../components/modelviews/confirmhostAdmin/Hostlist';
...
const VerifyModelsPage: React.FC = () => {
  const router = useRouter();
  const dispatch = useDispatch();

  const listofhost = useSelector((state: RootState) => state.model.Listofunverifiedhost);
  const unverifiedhoststatus = useSelector((state: RootState) => state.model.unverifiedhoststatus);
  ...
  const rejectmodelstatus = useSelector((state: RootState) => state.model.rejectmodelstatus);
  const verifymodelstatus = useSelector((state: RootState) => state.model.verifymodelstatus);
  ...
  useEffect(() => {
    if (rejectmodelstatus === 'succeeded' || verifymodelstatus === 'succeeded') {
      setLoading(false);
      setDisable(false);
      dispatch(changemodelstatus('idle'));
    }
  }, [rejectmodelstatus, verifymodelstatus, dispatch]);
...
export default VerifyModelsPage;
