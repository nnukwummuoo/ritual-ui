...
//         const newRemoteStream = event.streams[0];
//         console.log('📹 [WebRTC] Received remote stream:', newRemoteStream.id);
//         setRemoteStream(newRemoteStream);
...
//   // Handle accept call - FIXED: Now properly creates peer connection for both roles
//   const handleAcceptCall = async () => {
//     if (!callData || !socket) return;

//     console.log('🎯 [Debug] Call accepted - Role:', callData?.isIncoming ? 'answerer' : 'caller');
//     setCallStatus('connecting');
...
//     // Create peer connection for BOTH roles
//     const pc = createPeerConnection();
//     setPeerConnection(pc);
//     peerConnectionRef.current = pc;

//     // Add local tracks for BOTH roles
...
//     // If we're the caller, create and send offer immediately
//     if (!callData.isIncoming) {
...
//       thumbnailVideoRef.current.srcObject = localStream;
//       thumbnailVideoRef.current.play().catch(console.warn);
//     }
//   }, [localStream]);
...
//       console.log('📹 [VideoCall] Setting remote stream for main video');
//       mainVideoRef.current.srcObject = remoteStream;
...
//   // Set usersCanSeeEachOther when both streams are available
//   useEffect(() => {
//     if (localStream && remoteStream && remoteVideoEnabled) {
//       console.log('✅ [VideoCall] Users can now see each other');
//       setUsersCanSeeEachOther(true);
//     }
//   }, [localStream, remoteStream, remoteVideoEnabled]);

//   // SOCKET EVENT HANDLERS - COMPLETELY FIXED
//   useEffect(() => {
//     if (!socket || !isOpen) return;

//     const handleCallAccepted = async (data: any) => {
//       console.log('🎯 [Debug] Call accepted event - Role:', callData?.isIncoming ? 'answerer' : 'caller');
      
//       // For the caller: peer connection is already created in handleAcceptCall
//       // For the answerer: peer connection is already created in handleAcceptCall
      
//       setCallStatus('connected');
//     };

//     const handleOffer = async (data: any) => {
//       console.log('🎯 [Debug] Offer received - PC state:', peerConnectionRef.current?.signalingState);
      
//       if ((data.callId === callData?.callId || data.callId.startsWith('temp_')) && peerConnectionRef.current) {
//         console.log('📞 [VideoCall] Answerer processing offer');
...
//     const handleAnswer = async (data: any) => {
//       console.log('🎯 [Debug] Answer received - PC state:', peerConnectionRef.current?.signalingState);
...
//       if (shouldAccept && peerConnectionRef.current) {
//         try {
...
//       socket.off('insufficient_funds', handleInsufficientFundsFromServer);
//     };
//   }, [socket, isOpen, currentUserId, callData, handleCleanup, onClose, handleInsufficientFunds]);
...
//               <div className="absolute top-4 left-4 bg-black bg-opacity-70 text-white px-3 py-1 rounded text-xs">
//                 Connected
//               </div>
//             </div>
//           ) : (
...
//                       <div>Call ID: {callData?.callId}</div>
//                       <div>Status: {callStatus}</div>
//                       <div>Remote Video: {remoteVideoEnabled ? 'Enabled' : 'Disabled'}</div>
//                       <div>Peer Connection: {peerConnectionRef.current ? 'Active' : 'Inactive'}</div>
//                     </div>
//                   </div>
//                 )}
//               </div>
