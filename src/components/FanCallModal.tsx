...
export default function FanCallModal({
  ...
  const controlsTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  const callTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  const streamRecoveryRef = useRef<NodeJS.Timeout | null>(null);
  const [peerConnection, setPeerConnection] = useState<RTCPeerConnection | null>(null);
  const socket = getSocket();
  ...
  // COMPLETELY REVISED Peer Connection Creation
  const createPeerConnection = useCallback(() => {
    console.log('🔄 [WebRTC] Creating new peer connection');
    
    const pc = new RTCPeerConnection(rtcConfig);

    // CRITICAL FIX: Handle incoming tracks at BOTH stream and track level
    pc.ontrack = (event) => {
      ...
      if (event.streams && event.streams.length > 0) {
        const newRemoteStream = event.streams[0];
        console.log('📹 [WebRTC] Received remote stream via ontrack:', {
          streamId: newRemoteStream.id,
          tracks: newRemoteStream.getTracks().map(t => ({ kind: t.kind, id: t.id, enabled: t.enabled }))
        });

        // Enhanced stream monitoring
        newRemoteStream.onaddtrack = (trackEvent) => {
          console.log('📹 [WebRTC] Track added to remote stream:', trackEvent.track.kind);
          forceStreamAttachment(newRemoteStream);
        };

        newRemoteStream.onremovetrack = (trackEvent) => {
          console.log('📹 [WebRTC] Track removed from remote stream:', trackEvent.track.kind);
        };
        ...
        monitorRemoteTracks(newRemoteStream);
      }
    };
    ...
    pc.onconnectionstatechange = () => {
      ...
      if (pc.connectionState === 'connected') {
        ...
        // Start stream recovery monitoring
        startStreamRecoveryMonitoring();
      } else if (pc.connectionState === 'failed' || pc.connectionState === 'disconnected') {
        console.warn('⚠️ [WebRTC] Connection issues:', pc.connectionState);
      }
    };
  ...
  // CRITICAL: Force stream attachment to video element
  const forceStreamAttachment = useCallback((stream: MediaStream) => {
    if (!mainVideoRef.current) {
      console.warn('⚠️ [WebRTC] mainVideoRef not available for stream attachment');
      return;
    }

    console.log('📹 [WebRTC] Force attaching stream to main video:', {
      streamId: stream.id,
      tracks: stream.getTracks().length,
      currentSrcObject: !!mainVideoRef.current.srcObject
    });

    // Always update srcObject - don't check if it's the same
    mainVideoRef.current.srcObject = stream;
    
    // Multiple play attempts with increasing delays
    const attemptPlay = (attempt = 0, maxAttempts = 5) => {
      if (attempt >= maxAttempts) {
        console.warn('❌ [WebRTC] Max play attempts reached');
        return;
      }

      setTimeout(() => {
        if (mainVideoRef.current && mainVideoRef.current.paused) {
          console.log(`📹 [WebRTC] Play attempt ${attempt + 1}`);
          mainVideoRef.current.play()
            ...
            .catch((error) => {
              console.warn(`📹 [WebRTC] Play attempt ${attempt + 1} failed:`, error.message);
  ...
  // Monitor remote tracks for status changes
  const monitorRemoteTracks = useCallback((stream: MediaStream) => {
    const videoTracks = stream.getVideoTracks();
    const audioTracks = stream.getAudioTracks();
    
    console.log('📹 [WebRTC] Monitoring remote tracks:', {
    ...
    if (videoTracks.length > 0) {
      setRemoteVideoEnabled(videoTracks[0].readyState === 'live');
    }
  }, [forceStreamAttachment]);

  // Enhanced stream recovery monitoring
  const startStreamRecoveryMonitoring = useCallback(() => {
    if (streamRecoveryRef.current) {
      clearInterval(streamRecoveryRef.current);
    }

    streamRecoveryRef.current = setInterval(() => {
      const videoElement = mainVideoRef.current;
      const currentStream = remoteStream;

      if (!videoElement || !currentStream) return;

      // Check if video element has no srcObject but we have a stream
      if (!videoElement.srcObject && currentStream) {
        console.log('🔄 [WebRTC] Recovery: Reattaching lost stream');
        forceStreamAttachment(currentStream);
        return;
      }

      // Check if video is paused but should be playing
      if (videoElement.paused && currentStream.active) {
        console.log('🔄 [WebRTC] Recovery: Video paused but stream active, attempting play');
        videoElement.play().catch(e => {
          console.warn('📹 [WebRTC] Recovery play failed:', e.message);
        });
      }

      // Check video dimensions to see if video is actually displaying
      if (videoElement.videoWidth === 0 && videoElement.videoHeight === 0 && remoteVideoEnabled) {
        console.warn('⚠️ [WebRTC] Video element has 0x0 dimensions but remote video should be enabled');
        // Try reattaching the stream
        forceStreamAttachment(currentStream);
      }
    }, 2000);
  }, [remoteStream, remoteVideoEnabled, forceStreamAttachment]);
  ...
  // Enhanced local stream attachment
  useEffect(() => {
    if (localStream && thumbnailVideoRef.current) {
      console.log('📹 [VideoCall] Setting local stream for thumbnail');
      thumbnailVideoRef.current.srcObject = localStream;
      
      // Ensure thumbnail video plays
      const playThumbnail = () => {
        if (thumbnailVideoRef.current && thumbnailVideoRef.current.paused) {
          thumbnailVideoRef.current.play().catch(e => {
            if (e.name !== 'AbortError') {
              console.warn('📹 [VideoCall] Thumbnail play failed:', e.message);
            }
          });
        }
      };
      ...
      setTimeout(playThumbnail, 500);
    }
  }, [localStream]);

  // Enhanced remote stream attachment with recovery
  useEffect(() => {
    if (remoteStream && mainVideoRef.current) {
      console.log('📹 [VideoCall] Remote stream updated, forcing attachment');
      forceStreamAttachment(remoteStream);
    }
  }, [remoteStream, forceStreamAttachment]);

  // Set usersCanSeeEachOther when both streams are available
  useEffect(() => {
    if (localStream && remoteStream && remoteVideoEnabled && !usersCanSeeEachOther) {
      console.log('✅ [VideoCall] Users can now see each other');
      setUsersCanSeeEachOther(true);
    }
  }, [localStream, remoteStream, remoteVideoEnabled, usersCanSeeEachOther]);

  // SOCKET EVENT HANDLERS - COMPLETELY REVISED
  useEffect(() => {
    if (!socket || !isOpen) return;

    const handleCallAccepted = async (data: any) => {
      console.log('📞 [VideoCall] Call accepted event received');
      setCallStatus('connected');

      // Caller creates offer, answerer waits for offer
      if (data.callerId === currentUserId && !callData?.isIncoming) {
        console.log('📞 [VideoCall] I am the caller, creating offer');
        
        const pc = createPeerConnection();
        setPeerConnection(pc);

        // Add local tracks with validation
        if (localStream && localStream.getTracks().length > 0) {
          ...
          localStream.getTracks().forEach(track => {
            try {
              ...
              console.error(`❌ [WebRTC] Error adding ${track.kind} track:`, error);
            }
          });
        } else {
          console.error('❌ [WebRTC] No local stream tracks available');
          return;
        }

        // Create offer with video codec preferences
        try {
          ...
          console.log('📞 [WebRTC] Created local offer');
    ...
    const handleOffer = async (data: any) => {
      console.log('📞 [VideoCall] Received offer from remote peer');
      
      if ((data.callId === callData?.callId || data.callId.startsWith('temp_')) && !peerConnection && callData?.isIncoming) {
        console.log('📞 [VideoCall] I am the answerer, processing offer');
    ...
    const handleAnswer = async (data: any) => {
      console.log('📞 [VideoCall] Received answer from remote peer');
    ...
    const handleInsufficientFundsFromServer = (data: any) => {
      console.log('💰 [VideoCall] Insufficient funds received from server');
      const isCaller = callData?.callerId === currentUserId;
      if ((data.callId === callData?.callId || !data.callId) && isCaller) {
        handleInsufficientFunds();
      }
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-black bg-opacity-90 flex items-center justify-center">
      <div className="w-full h-full flex flex-col">
        ...
        {/* Main Video Display */}
        <div className="flex-1 relative bg-gray-900 cursor-pointer" onClick={handleVideoAreaClick}>
          {/* Main video display - Enhanced with better state management */}
          {remoteStream && remoteVideoEnabled ? (
            <div className="w-full h-full relative">
              <video
                ...
                onLoadStart={() => console.log('📹 [VideoCall] Main video load started')}
                onCanPlay={() => {
                  console.log('📹 [VideoCall] Main video can play event');
                  if (mainVideoRef.current) {
                    mainVideoRef.current.play().catch(e => {
                      console.warn('📹 [VideoCall] Main video play in canplay failed:', e.message);
                    });
                  }
                }}
                onPlay={() => {
                  console.log('✅ [VideoCall] Main video started playing');
                  setRemoteVideoEnabled(true);
                }}
                onStalled={() => console.warn('⚠️ [VideoCall] Main video stalled')}
                onWaiting={() => console.log('📹 [VideoCall] Main video waiting')}
              />
              
              {/* Video status indicator */}
              {/* <div className="absolute top-4 left-4 bg-black bg-opacity-50 text-white px-3 py-1 rounded text-sm">
                {remoteVideoEnabled ? 'Remote Video Live' : 'Remote Video Off'}
              </div> */}
            </div>
          ) : (
            <div className="w-full h-full flex items-center justify-center">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-3">
                      ...
                      <p className="text-sm text-gray-400">
                        ...
                        {callStatus === 'connected' && !remoteVideoEnabled && ' (Waiting for video...)'}
                      </p>
                    </div>
        ...
        {/* Picture in Picture */}
        {localStream && (
          <div className="absolute bottom-20 right-4 w-24 h-36 bg-gray-800 rounded-lg overflow-hidden border-2 border-white shadow-lg">
            <video
              ref={thumbnailVideoRef}
              autoPlay
              playsInline
