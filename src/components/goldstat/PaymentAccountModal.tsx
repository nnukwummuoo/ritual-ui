...
import CountrySelect from "../CountrySelect/CountrySelect"; // Import CountrySelect component
...
const PaymentAccountModal: React.FC<PaymentAccountModalProps> = ({ accesstoken, userId }) => {
  ...
  const renderInput = (
    ...
    <div className="flex flex-col gap-1">
      <label className="text-left text-sm text-gray-700">{placeholder}</label>
      <input
        type={type}
        name={name}
        required={required}
        placeholder={placeholder}
        className="border rounded-md text-black px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#F97316]"
  ...
  const renderSelect = (
    ...
    <div className="flex flex-col gap-1">
      <label className="text-left text-sm text-gray-700">{label}</label>
      <select
        name={name}
        required
        className="border text-black rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#F97316]"
        onChange={(e) => setFormData({ ...formData, [name]: e.target.value })}
      >
  ...
  return (
    <>
      <div className="bg-gray-800 w-full px-4 py-3 rounded-md shadow-sm mb-3">
        <div className="w-full flex flex-wrap items-center justify-between gap-2">
          ...
          {account ? (
            ...
            <button
              ...
              className="bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 px-4 py-2 rounded-2xl text-white text-xs font-medium active:bg-[#F9731670] hover:bg-[#F97316a7] disabled:opacity-50"
            >
              Add Account
            </button>
          )}
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center px-4">
          <div className="bg-white rounded-xl w-full max-w-sm p-6 text-center shadow-xl">
            <form onSubmit={handleSubmit} className="flex flex-col gap-3 text-left">
              <h2 className="text-lg font-bold mb-3 text-center text-[#F97316]">
                Cryptocurrency Account Info
              </h2>
              ...
              <div className="flex flex-col gap-1">
                <label className="text-left text-sm text-gray-700">Country of Residence</label>
                <CountrySelect onSelectCountry={(country) => setFormData({ ...formData, country })} />
              </div>
              {renderSelect("currency", "Preferred Currency", [
              ...
              {renderSelect("cryptoType", "Cryptocurrency Type", [
                ...
                { value: "USDT_ERC20", label: "USDT (Tether, ERC20)" },
              ])}
              ...
              <button
                type="submit"
                className="bg-[#F97316] text-white rounded-md py-2 hover:bg-[#f97316d6] disabled:bg-gray-400"
                disabled={loading}
              >
