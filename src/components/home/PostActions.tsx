...
//       strokeWidth={1.5}
//       className={`${iconBase} ${filled ? 'text-white drop-shadow-sm scale-110' : 'text-gray-400 hover:text-gray-200'} transition-all duration-200`}
//     >
//       <path
//         strokeLinecap="round"
//         strokeLinejoin="round"
...
//           <div className="block w-full text-left px-4 py-2 hover:bg-gray-700 rounded text-white" onClick={async () => { /* Implement delete functionality here */
...
//       <button
//         type="button"
//         className={`flex items-center gap-2 px-3 py-2 rounded hover:bg-gray-700/50 ${liked ? 'text-white' : ''} transition-colors duration-200`}
//         onClick={() => {
//           if (onLike) onLike();
//         }}
//         disabled={isLiking}
...
function StarIconComponent({ filled }: { filled?: boolean }) {
  return filled ? (
    <Image src={StarIcon2} alt="star filled" width={24} height={24} className={iconBase} />
  ) : (
    <Image src={StarIcon} alt="star outline" width={24} height={24} className={iconBase} />
  );
}
    ...
    {hasPop ? <div className="d-flex fixed bg-[#0e0e0e80] top-0 h-[100dvh] bottom-0 left-0 right-0" onClick={(e) => { 
      ...
      {own&&<>
        <div className="block w-full text-left px-4 py-2 hover:bg-gray-700 rounded" onClick={() => { setHasPop(false); router.push(`/post/${post?._id}/edit`) }}>Edit Post</div>
          ...
          <div className="block w-full text-left px-4 py-2 hover:bg-gray-700 rounded text-white" onClick={async () => {
            const tst=toast.loading("Deleting")
            try {
               setHasPop(false);
            await deletesinglepost(post?._id);
