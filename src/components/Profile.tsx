...
export default function Profile({ name, firstname, lastname, src, url, gold_balance, pending_balance, isVip, vipEndDate, verified, onClick }:
  { name: string, firstname?: string, lastname?: string, src: string, url: string, gold_balance: number | string, pending_balance?: number | string, isVip?: boolean, vipEndDate?: string, verified?: boolean, onClick?: () => void }) {
  ...
  return <Link href={url} className={`flex gap-4 flex-col w-full`} onClick={(e) => {
    ...
    <div className={`flex gap-4 group w-full`}>
      ...
      <div className='flex flex-col gap-1 font-bold w-full'>
        <p className=" group-hover:text-gray-400 text-xl flex items-center gap-1">
          {firstname && lastname ? `${firstname} ${lastname}` : name}
          {verified && <BadgeCheck size={18} fill="white" className="text-black" />}
        </p>
        <p className=" group-hover:text-gray-400 text-blue-400">{isVip ? "VIP" : "Basic Member"}</p>
        <p ><span>Gold bal:</span> <span className='text-white font-bold text-[13px] '>{gold_balance}</span></p>
        {pending_balance && Number(pending_balance) > 0 && (
          <p className="text-orange-400 text-xs">
            <span>Pending:</span> <span className='text-orange-500 font-bold'>{pending_balance}</span>
          </p>
        )}
